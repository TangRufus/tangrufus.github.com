...
var $slider=$('.flexslider');if($slider.length){$slider.waitForImages(function(){$slider.addClass('ready').flexslider({animation:"slide",controlsContainer:".flex-container",slideshowSpeed:slideTime,animationDuration:animSpeed,start:function(slider){var $new_height=slider.slides.eq(0).height();slider.slides.removeClass('current');slider.slides.eq(0).addClass('current');setTimeout(doResizeSlide,500);},before:function(slider){var $animatingTo=slider.slides.eq(slider.animatingTo);var $new_height=slider.slides.eq(slider.animatingTo).height();slider.slides.removeClass('current');$animatingTo.addClass('current');if($new_height!=slider.height()){slider.stop().animate({height:$new_height},250);}}});},null,true);}
$(window).resize(function(){clearTimeout(this.id);this.id=setTimeout(doResizeSlide,500);})
function doResizeSlide(){var slider=jQuery('.flexslider');var newHeight=jQuery('li.current',slider).height();slider.stop().animate({height:newHeight});}
var $container=$('#portfolio');if($container.length){$container.waitForImages(function(){$container.isotope({itemSelector:'.item',layoutMode:'fitRows'});$('#filters a').click(function(){var selector=$(this).attr('data-filter');$container.isotope({filter:selector});$(this).parent().addClass('current').siblings().removeClass('current');return false;});},null,true);}});
$(document).ready(function(){$("a").each(function(){if(this.href.indexOf(location.hostname)==-1){$(this).attr({target:"_blank",});}})});